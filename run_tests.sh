...
function usage {
  echo "Usage: $0 [OPTION]..."
  echo "Run OnMetal-Scripts's test suite(s)"
  echo ""
...
function run_tests {
  ...
  if [ $debug -eq 1 ]; then
    if [ "$testropts" = "" ] && [ "$testrargs" = "" ]; then
      ...
      # provided.
      testrargs="discover ./onmetal_scripts/tests"
    fi
  ...
  if [ setup.cfg -nt onmetal_scripts.egg-info/entry_points.txt ]
  then
  ...
  if [ $coverage -eq 1 ]; then
    ...
    ${wrapper} coverage html --include='onmetal_scripts/*' --omit='onmetal_scripts/openstack/common/*' -d covhtml -i
  fi
